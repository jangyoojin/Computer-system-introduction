...
//Explicit free list 구현을 위한 mecro function
#define PREV_PTR(p) ((char*)p) //prev pointer of free block
#define NEXT_PTR(p) ((char*)(P) + WSIZE) // next pointer of free block
#define MOVE_PREV(p) (*(char**)(p)) //이전 free block으로 점프
#define MOVE_NEXT(p) (*(char**)(NEXT_PTR(p))) // 다음 free block으로 점프
...
static void* find_fit(size_t asize){
    void* bp;
    for(bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)){//heap의 처음부터 끝까지 모든 block을 돌면서
